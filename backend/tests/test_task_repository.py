...
class TestTaskRepository:
    ...
    async def test_get_all_tasks_by_user(self, task_repo: TaskRepository, test_user: uuid.UUID):
        ...
        for i, task_data in enumerate(tasks_data):
            task = await task_repo.create(user_id=test_user, **task_data)
    ...
    async def test_update_nonexistent_task(self, task_repo: TaskRepository):
        ...
        fake_id = uuid.uuid4()
        update_data = TaskUpdateRequest(title="New Title")
...
class TestTaskStepRepository:
    ...
    async def test_get_steps_by_task(self, step_repo: TaskStepRepository, db_session: AsyncSession, test_user: uuid.UUID):
        ...
        for step_data in steps_data:
            await step_repo.create(task.id, step_data)
    ...
    async def test_delete_step(self, step_repo: TaskStepRepository, db_session: AsyncSession, test_user: uuid.UUID):
        ...
        task = await task_repo.create(
            user_id=test_user,
            title="Task with step to delete",
            description="Test task"
        )
